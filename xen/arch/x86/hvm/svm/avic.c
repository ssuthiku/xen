...
int svm_avic_dom_init(struct domain *d)
{
    ...
    /* Here, we need to setup the write permission in the guest p2m entry
     * for the APIC_DEFAULT_PHYS_BASE. HW doesn't care about the actual page.
     */
    if ( !d->arch.hvm_domain.svm.avic_access_page_done )
    {
        set_mmio_p2m_entry(d, paddr_to_pfn(APIC_DEFAULT_PHYS_BASE), _mfn(0),
...
//	    printk("DEBUG: %s: AFTER rip=%#llx\n", __func__, (unsigned long long)regs->eip);
    ...
    default:
        ...
        {
//SURAVEE: HACK:
//ffffffff81055c76:       89 b7 00 d0 5f ff       mov    %esi,-0xa03000(%rdi)
	    __update_guest_eip(regs, 6);
//	    printk("DEBUG: %s: Fault Write after rip=%#llx\n", __func__, (unsigned long long)regs->eip);
        }
        else
        {
